...
class RXNGRegressor(torch.nn.Module):
    def __init__(self,gnum_layer,tnum_layer,onum_layer,emb_dim,JK="last",output_size=1,drop_ratio=0.0,
                 ...
                 rel_pos_buckets=11,rel_pos="emb_only",forward_compat_mode="modern",pretrained_encoder=None,pretrained_rct_encoder=None,pretrained_pdt_encoder=None,
                 ...
                 rct_norm: NormType | str | None = None,pdt_norm: NormType | str | None = None,mid_norm: NormType | str | None = None,
        ...
        if not self.split_process:
            ...
            if pretrained_encoder is None:
                self.encoder = RXNGEncoder(gnum_layer,tnum_layer,emb_dim,JK=JK,drop_ratio=drop_ratio,attn_drop_ratio=attn_drop_ratio,
                    ...
                    rel_pos=rel_pos,task="retrosynthesis",forward_compat_mode=self.forward_compat_mode,
            ...
            self.decoder = RegressorLayer(hidden_size=self.emb_dim,output_size=output_size,layer_num=onum_layer,
                                        batch_norm=output_norm,act_func=self.output_act_func,norm_type=self.head_norm)
        else:
            ...
            if self.use_mid_inf:
                ...
                self.mid_decoder = RegressorLayer(hidden_size=mid_emb_dim,output_size=output_size,layer_num=onum_layer,
                                                batch_norm=output_norm,act_func=self.output_act_func,norm_type=self.head_norm) ## TODO
    ...
    def forward(self,data):
        ...
        if not self.split_process:
            ...
            if not self.use_mid_inf:
                ...
                if self.trans_readout == 'mean':
                    rct_rxn_transf_emb_merg = sequence_mean(rct_rxn_transf_emb, rct_memory_lengths, self.forward_compat_mode)
                    pdt_rxn_transf_emb_merg = sequence_mean(pdt_rxn_transf_emb, pdt_memory_lengths, self.forward_compat_mode)
                else:
                    raise NotImplementedError(f"Unsupported trans_readout: {self.trans_readout}")
                ...
                if self.trans_readout == 'mean':
                    rct_rxn_transf_emb_merg = sequence_mean(rct_rxn_transf_emb, rct_memory_lengths, self.forward_compat_mode)
                    pdt_rxn_transf_emb_merg = sequence_mean(pdt_rxn_transf_emb, pdt_memory_lengths, self.forward_compat_mode)
                    mid_rxn_transf_emb_merg = sequence_mean(mid_rxn_transf_emb, mid_memory_lengths, self.forward_compat_mode)
                else:
                    raise NotImplementedError(f"Unsupported trans_readout: {self.trans_readout}")
...
class RXNGClassifier(torch.nn.Module):
    def __init__(self,gnum_layer,tnum_layer,onum_layer,emb_dim=256,JK="last",output_size=2,drop_ratio=0.0,
                 num_heads=4,gnn_type="gcn",bond_feat_red="mean",gnn_aggr='add',node_readout='sum',
                 trans_readout='mean',graph_pooling='attention',attn_drop_ratio=0.0,encoder_filter_size=2048,
                 rel_pos_buckets=11,rel_pos="emb_only",forward_compat_mode="modern",split_process=False,split_merge_method="all",output_act_func='relu',
                 rct_norm: NormType | str | None = "batchnorm",pdt_norm: NormType | str | None = "batchnorm",head_norm: NormType | str | None = "batchnorm"):
        super().__init__()
        self.emb_dim = emb_dim
        self.trans_readout = trans_readout
        self.split_process = split_process
        ...
        if not self.split_process:
            ...
            self.encoder = RXNGEncoder(gnum_layer,tnum_layer,emb_dim,JK=JK,drop_ratio=drop_ratio,attn_drop_ratio=attn_drop_ratio,
                    ...
                    rel_pos=rel_pos,task="retrosynthesis",forward_compat_mode=self.forward_compat_mode,
                    norm_type=self.rct_norm)
            self.decoder = ClassifierLayer(hidden_size=self.emb_dim,output_size=output_size,layer_num=onum_layer,act_func=self.output_act_func,norm_type=self.head_norm)
        else:
            self.rct_encoder = RXNGEncoder(gnum_layer,tnum_layer,emb_dim,JK=JK,drop_ratio=drop_ratio,attn_drop_ratio=attn_drop_ratio,
                    num_heads=num_heads,gnn_type=gnn_type,bond_feat_red=bond_feat_red,gnn_aggr=gnn_aggr,node_readout=node_readout,
                    graph_pooling=graph_pooling,encoder_filter_size=encoder_filter_size,rel_pos_buckets=rel_pos_buckets,enc_pos_encoding=None,
            ...
            if self.split_merge_method == "all":
                self.decoder = ClassifierLayer(hidden_size=self.emb_dim*3,output_size=output_size,layer_num=onum_layer,act_func=self.output_act_func,norm_type=self.head_norm) # *3 -> rct, pdt, diff
            elif self.split_merge_method == "only_diff":
                self.decoder = ClassifierLayer(hidden_size=self.emb_dim*1,output_size=output_size,layer_num=onum_layer,act_func=self.output_act_func,norm_type=self.head_norm) # *1 -> diff
            elif self.split_merge_method == "rct_pdt":
                self.decoder = ClassifierLayer(hidden_size=self.emb_dim*2,output_size=output_size,layer_num=onum_layer,act_func=self.output_act_func,norm_type=self.head_norm) # *2 -> rct, pdt
