...
from .model_factory import build_regression_model, build_sequence_model
from .regression_workflow import RegressionEvaluationSettings, evaluate_regression_split
...
class SeqEval:
    def __init__(
        ...
        trained_config = load_config(resolve_config_file(trained_model_path))
        vocab = load_vocab(
        ...
        CheckpointAdapter().load_into_model(
            model, ckpt_file, map_location=device, mode="strict"
        )
        self.model = model
...
def reaction_prediction(
    ...
    if task_type in ["reactivity", "selectivity"]:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with open(
                os.path.join(tmp_dir, "tgt_tokenized_smiles.txt"), "w"
            ) as fw:  # just a placeholder
                fw.writelines("\n".join(tokenized_smiles_lst))
            pretrained_config = load_config(resolve_config_file(model_path))
            shutil.copyfile(
                f"{pretrained_config.data.data_path}/{pretrained_config.data.vocab_file}",
