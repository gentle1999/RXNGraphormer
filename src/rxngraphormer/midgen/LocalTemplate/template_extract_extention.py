...
def get_special_groups(mol: Chem.Mol):
    ...
    # Define templates
    group_templates: list[tuple[Sequence[int] | tuple[int, ...], str]] = [
        # Functional groups
        (
            range(2),
            "[*]=[*]",
        ...
        (
            (6,),
            "CC(C)(C)OC(N)=O",
        ),  # Boc with non-aromatic nitrogen
        (
            (6,),
            "CC(C)(C)OC(n)=O",
        ),  # Boc with aromatic nitrogen
        (
            range(2),
            "NC(=O)OCc1ccccc1",
        ),
        (
            range(2),
            "NC(=O)OCC1c2ccccc2c2ccccc21",
        ),
    ]

    # Build list
    groups: list[tuple[list[int], tuple[int, ...], str]] = []
    for add_if_match, template in group_templates:
        matches: tuple[tuple[int, ...], ...] = mol.GetSubstructMatches(Chem.MolFromSmarts(template), useChirality=True)
...
def get_strict_smarts_for_special_atom(atom: Chem.Atom):
    """
