from collections import defaultdict
from collections.abc import Sequence
from typing import Any, cast

from rdkit import Chem

AtomMap = int
BondTuple = tuple[AtomMap, AtomMap]
EditNumlist = list[int | str]
ReplacementDict = dict[int, int]
bond_type_map: dict[str, str] = {
    "SINGLE": "-",
    "DOUBLE": "=",
    "TRIPLE": "#",
    "AROMATIC": "@",
}


def get_template_bond(temp_order: Sequence[int], bond_smarts):
    bond_match = {}
    for n, _ in enumerate(temp_order):
        bond_match[(temp_order[n], temp_order[n - 1])] = bond_smarts[n - 1]
        bond_match[(temp_order[n - 1], temp_order[n])] = bond_smarts[n - 1]
    return bond_match


def bond_to_smiles(bond: Chem.Bond):
    a1_label = str(bond.GetBeginAtom().GetAtomicNum())
    a2_label = str(bond.GetEndAtom().GetAtomicNum())
    if bond.GetBeginAtom().HasProp("molAtomMapNumber"):
        a1_label += bond.GetBeginAtom().GetProp("molAtomMapNumber")
    if bond.GetEndAtom().HasProp("molAtomMapNumber"):
        a2_label += bond.GetEndAtom().GetProp("molAtomMapNumber")
    atoms = sorted([a1_label, a2_label])
    bond_smarts = bond_type_map[str(bond.GetBondType())]
    return f"{atoms[0]}{bond_smarts}{atoms[1]}"


def check_bond_break(bond1: Chem.Bond | None, bond2: Chem.Bond | None):
    if bond1 is None and bond2 is not None:
        return False
    elif bond1 is not None and bond2 is None:
        return True
    else:
        return False


def check_bond_formed(bond1: Chem.Bond | None, bond2: Chem.Bond | None):
    if bond1 is not None and bond2 is None:
        return False
    elif bond1 is None and bond2 is not None:
        return True
    else:
        return False


def check_bond_change(pbond: Chem.Bond | None, rbond: Chem.Bond | None):
    if pbond is None or rbond is None:
        return False
    elif bond_to_smiles(pbond) != bond_to_smiles(rbond):
        return True
    else:
        return False


def atom_neighbors(atom: Chem.Atom):
    neighbor: list[int] = []
    for n in atom.GetNeighbors():
        neighbor.append(cast(Chem.Atom, n).GetAtomMapNum())
    return sorted(neighbor)


def extend_changed_atoms(changed_atom_tags: list[str], reactants: list[Chem.Mol], max_map: int):
    for reactant in reactants:
        extend_idx = []
        for atom_idx in range(reactant.GetNumAtoms()):
            atom = reactant.GetAtomWithIdx(atom_idx)
            if str(atom.GetAtomMapNum()) in changed_atom_tags:
                for n in atom.GetNeighbors():
                    n = cast(Chem.Atom, n)
                    if n.GetAtomMapNum() == 0:
                        extend_idx.append(n.GetIdx())
        for idx in extend_idx:
            reactant.GetAtomWithIdx(idx).SetAtomMapNum(max_map)


def check_atom_change(patom: Chem.Atom, ratom: Chem.Atom):
    return atom_neighbors(patom) != atom_neighbors(ratom)


def label_retro_edit_site(
    products: str, reactants: str, edit_num: EditNumlist
) -> tuple[list[int], list[BondTuple], list[BondTuple], list[int]]:
    edit_num_int: list[int] = [int(num) for num in edit_num]
    pmol = Chem.MolFromSmiles(products)
    rmol = Chem.MolFromSmiles(reactants)
    patom_map = {pmol.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(pmol.GetNumAtoms())}
    ratom_map = {rmol.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(rmol.GetNumAtoms())}
    used_atom = set()
    grow_atoms = []
    broken_bonds = []
    changed_bonds = []

    # cut bond
    for a in edit_num_int:
        for b in edit_num_int:
            if a >= b:
                continue
            pbond = pmol.GetBondBetweenAtoms(patom_map[a], patom_map[b])
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            if check_bond_break(pbond, rbond):  # cut bond
                broken_bonds.append((a, b))
                used_atom.update([a, b])

    # Add LG
    for a in edit_num_int:
        if a in used_atom:
            continue
        patom = pmol.GetAtomWithIdx(patom_map[a])
        ratom = rmol.GetAtomWithIdx(ratom_map[a])
        if check_atom_change(patom, ratom):
            used_atom.update([a])
            grow_atoms.append(a)

    # change bond type
    for a in edit_num_int:
        for b in edit_num_int:
            if a >= b:
                continue
            pbond = pmol.GetBondBetweenAtoms(patom_map[a], patom_map[b])
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            if check_bond_change(pbond, rbond):
                if a not in used_atom and b not in used_atom:
                    changed_bonds.append((a, b))

    used_atoms = set(grow_atoms + [atom for bond in broken_bonds + changed_bonds for atom in bond])
    remote_atoms = [atom for atom in edit_num_int if atom not in used_atoms]
    remote_atoms_ = []
    for a in remote_atoms:
        atom = rmol.GetAtomWithIdx(ratom_map[a])
        neighbors_map = [n.GetAtomMapNum() for n in atom.GetNeighbors()]
        connected_neighbors = [b for b in used_atoms if b in neighbors_map]
        if len(connected_neighbors) == 0:
            remote_atoms_.append(a)

    return grow_atoms, broken_bonds, changed_bonds, remote_atoms_


def label_foward_edit_site(
    reactants: str, products: str, edit_num: EditNumlist
) -> tuple[list[BondTuple], list[BondTuple], list[BondTuple], list[BondTuple]]:
    edit_num_int = [int(num) for num in edit_num]
    rmol = Chem.MolFromSmiles(reactants)
    pmol = Chem.MolFromSmiles(products)
    patom_map = {pmol.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(pmol.GetNumAtoms())}
    ratom_map = {rmol.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(rmol.GetNumAtoms())}
    formed_bonds = []
    broken_bonds = []
    changed_bonds = []
    acceptors1 = set()
    acceptors2 = set()
    donors = set()

    # cut bond
    for a in edit_num_int:
        for b in edit_num_int:
            if a >= b:
                continue
            try:
                pbond = pmol.GetBondBetweenAtoms(patom_map[a], patom_map[b])
            except Exception:
                pbond = None
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            if check_bond_break(rbond, pbond):
                if a in patom_map:
                    broken_bonds.append((a, b))
                    acceptors1.add(a)
                if b in patom_map:
                    broken_bonds.append((b, a))
                    acceptors1.add(b)

    # change bond
    for a in edit_num_int:
        for b in edit_num_int:
            if a >= b:
                continue
            try:
                pbond = pmol.GetBondBetweenAtoms(patom_map[a], patom_map[b])
            except Exception:
                pbond = None
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            if check_bond_change(rbond, pbond):
                changed_bonds.append((a, b))
                changed_bonds.append((b, a))
                acceptors2.update([a, b])

    symmetric = True
    # form bond
    for a in edit_num_int:
        for b in edit_num_int:
            if a >= b:
                continue
            try:
                pbond = pmol.GetBondBetweenAtoms(patom_map[a], patom_map[b])
            except Exception:
                pbond = None
            rbond = rmol.GetBondBetweenAtoms(ratom_map[a], ratom_map[b])
            if check_bond_formed(rbond, pbond):  # cut bond
                if a not in acceptors1 and b not in acceptors1 and a not in acceptors2 and b not in acceptors2:
                    formed_bonds.append((a, b))
                    formed_bonds.append((b, a))
                elif a in acceptors1 and b in acceptors1:
                    symmetric = False
                    formed_bonds.append((a, b))
                    formed_bonds.append((b, a))
                else:
                    symmetric = False
                    if a in acceptors1:
                        formed_bonds.append((b, a))
                    elif a in acceptors2 and b not in acceptors1:
                        formed_bonds.append((b, a))
                    if b in acceptors1:
                        formed_bonds.append((a, b))
                    elif b in acceptors2 and a not in acceptors1:
                        formed_bonds.append((a, b))

    if not symmetric:
        new_changed_bonds = []
        # electron acceptor propagation
        acceptors = set([bond[1] for bond in formed_bonds]).union(acceptors1)
        for atom in acceptors:
            for bond in changed_bonds:
                if bond[0] == atom:
                    new_changed_bonds.append(bond)
        donors = set([bond[0] for bond in formed_bonds])
        for atom in donors:
            for bond in changed_bonds:
                if bond[1] == atom:
                    new_changed_bonds.append(bond)
        changed_bonds = list(set(new_changed_bonds))

    used_atoms = set([atom for bond in formed_bonds + broken_bonds + changed_bonds for atom in bond])
    remote_atoms = [atom for atom in edit_num_int if atom not in used_atoms]
    remote_bonds = []
    for a in remote_atoms:
        atom = rmol.GetAtomWithIdx(ratom_map[a])
        neighbors_map = [n.GetAtomMapNum() for n in atom.GetNeighbors()]
        connected_neighbors = [b for b in used_atoms if b in neighbors_map]
        if len(connected_neighbors) > 0:
            pass
        else:
            for n in neighbors_map:
                remote_bonds.append((a, n))

    return formed_bonds, broken_bonds, changed_bonds, remote_bonds


def label_CH_change(
    smiles1: str, smiles2: str, edit_num: EditNumlist, replacement_dict: ReplacementDict
) -> tuple[dict[int, int], dict[int, int], dict[int, int]]:
    mol1, mol2 = Chem.MolFromSmiles(smiles1), Chem.MolFromSmiles(smiles2)
    atom_map_dict1 = {mol1.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(mol1.GetNumAtoms())}
    atom_map_dict2 = {mol2.GetAtomWithIdx(atom_idx).GetAtomMapNum(): atom_idx for atom_idx in range(mol2.GetNumAtoms())}
    H_dict: defaultdict[int, dict[str, int]] = defaultdict(dict)
    C_dict: defaultdict[int, dict[str, int]] = defaultdict(dict)
    for atom_map in edit_num:
        atom_map = int(atom_map)
        if atom_map in atom_map_dict2:
            atom1, atom2 = mol1.GetAtomWithIdx(atom_map_dict1[atom_map]), mol2.GetAtomWithIdx(atom_map_dict2[atom_map])
            H_dict[atom_map]["smiles1"], C_dict[atom_map]["smiles1"] = (
                atom1.GetNumExplicitHs(),
                int(atom1.GetFormalCharge()),
            )
            H_dict[atom_map]["smiles2"], C_dict[atom_map]["smiles2"] = (
                atom2.GetNumExplicitHs(),
                int(atom2.GetFormalCharge()),
            )
    H_change = {replacement_dict[k]: v["smiles2"] - v["smiles1"] for k, v in H_dict.items()}
    Charge_change = {replacement_dict[k]: v["smiles2"] - v["smiles1"] for k, v in C_dict.items()}
    return atom_map_dict1, H_change, Charge_change


def bondmap2idx(
    bond_maps: list[BondTuple],
    idx_dict: dict[int, int],
    temp_dict: dict[int, int],
    remote: bool = False,
) -> tuple[
    list[tuple[int, int]],
    list[BondTuple],
    list[tuple[int, int]] | list[tuple[int, int]],
]:
    bond_idxs = [(idx_dict[bond_map[0]], idx_dict[bond_map[1]]) for bond_map in bond_maps]
    if remote:
        bond_temps = list(set([(temp_dict[bond_map[0]], -1) for bond_map in bond_maps]))
    else:
        bond_temps = [(temp_dict[bond_map[0]], temp_dict[bond_map[1]]) for bond_map in bond_maps]
    return (bond_idxs, bond_maps, bond_temps)


def atommap2idx(
    atom_maps: list[int], idx_dict: dict[int, int], temp_dict: dict[int, int]
) -> tuple[list[int], list[int], list[int]]:
    atom_idxs = [idx_dict[atom_map] for atom_map in atom_maps]
    atom_temps = [temp_dict[atom_map] for atom_map in atom_maps]
    return (atom_idxs, atom_maps, atom_temps)


def match_label(
    reactants: str,
    products: str,
    replacement_dict: dict[Any, Any],  # Allow flex input, convert inside
    edit_num: EditNumlist,
    retro: bool = True,
    remote: bool = True,
) -> tuple[dict[str, Any], dict[int, int], dict[int, int]]:
    if retro:
        smiles1 = products
        smiles2 = reactants
    else:
        smiles1 = reactants
        smiles2 = products

    replacement_dict = {int(k): int(v) for k, v in replacement_dict.items()}
    atom_map_dict, H_change, Charge_change = label_CH_change(smiles1, smiles2, edit_num, replacement_dict)

    if retro:
        ALG_atoms, broken_bonds, changed_bonds, remote_atoms = label_retro_edit_site(smiles1, smiles2, edit_num)
        edits = {
            "A": atommap2idx(ALG_atoms, atom_map_dict, replacement_dict),
            "B": bondmap2idx(broken_bonds, atom_map_dict, replacement_dict),
            "C": bondmap2idx(changed_bonds, atom_map_dict, replacement_dict),
        }
        if remote:
            edits["R"] = atommap2idx(remote_atoms, atom_map_dict, replacement_dict)
    else:
        formed_bonds, broken_bonds, changed_bonds, remote_bonds = label_foward_edit_site(smiles1, smiles2, edit_num)
        edits = {
            "A": bondmap2idx(formed_bonds, atom_map_dict, replacement_dict),
            "B": bondmap2idx(broken_bonds, atom_map_dict, replacement_dict),
            "C": bondmap2idx(changed_bonds, atom_map_dict, replacement_dict),
        }
        if remote:
            edits["R"] = bondmap2idx(remote_bonds, atom_map_dict, replacement_dict, True)
    return edits, H_change, Charge_change
