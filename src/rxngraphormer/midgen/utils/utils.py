...
def clean_leaving_mapping(rxn: str) -> tuple[str, dict[int, int]]:
    r, p = rxn.split(">>")
    p = max(p.split("."), key=len)
    rmol, pmol = Chem.MolFromSmiles(r), Chem.MolFromSmiles(p)
    rmaps = {atom.GetIdx(): atom.GetAtomMapNum() for atom in rmol.GetAtoms()}  # type: ignore
    pmaps = [atom.GetAtomMapNum() for atom in pmol.GetAtoms()]  # type: ignore
    [
        atom.SetAtomMapNum(0)
        for atom in rmol.GetAtoms()  # type: ignore
        if atom.GetAtomMapNum() not in pmaps
    ]
    r = Chem.MolToSmiles(rmol)
    return f"{Chem.MolToSmiles(rmol)}>>{p}", rmaps


def bond_to_label(bond: Bond) -> str:
    ...
    return f"{atoms[0]}{bond.GetSmarts()}{atoms[1]}"  # type: ignore


def atoms_are_different(atom1: Atom, atom2: Atom) -> bool:
...
def find_map_num(mol: Mol, mapnum: int) -> tuple[int, Atom]:
    return [
        (a.GetIdx(), a)
...
def get_map_num_from_index(mol: Mol, i: int) -> int:
    return mol.GetAtomWithIdx(i).GetAtomMapNum()


def get_symbol_from_map_num(mol: Mol, i: int) -> str:
    return find_map_num(mol, i)[-1].GetSymbol()


def get_neighbor_props(mol: Mol, atom_tag: int, prop: str) -> list[int] | list[str]:
...
def get_map_numbered_matches(mol: Mol, template: str) -> list[tuple[int, ...]]:
    """Given a molecule (mol) and target template, this funnction returns
    list of matches in the molecule indicated by map numbers"""
    return [
        tuple(map(lambda i: get_map_num_from_index(mol, i), match))
        for match in mol.GetSubstructMatches(Chem.MolFromSmarts(template))
    ]
...
def get_neighboring_hydrogens(mol: Mol, atom_map_num: int) -> list[int]:
...
def get_indexed_ss_seq(mol: Mol, map_numbered_ss_seq: list[Any]) -> list[Any]:
    ...
    def update(s):
        if isinstance(s, int):
            ...
            return tuple(contents) if type(s) == tuple else list(contents)  # noqa: E721
...
def get_num_unshared_el(mol: Mol, idx: int) -> int:
    """Given a molecule(mol) and index of an atom (idx),
    this function returns the number of unshared electrons around that atom"""
    Chem.Kekulize(
        mol
    )  # to get exact number of bonds in aromatic rings  (double and single bonds instead of 1.5 bond orders)
...
def compare_neighbors(atom_tag: int, mol_1: Mol, mol_2: Mol, prop: str) -> bool:
...
def get_bond_type(mol: Mol, atom_tag_1: int, atom_tag_2: int) -> float:
...
def apply_seq(rxn: str, map_numbered_ss_seq: list[Any]) -> list[Mol] | None:
    """Given a reaction (rxn) and its label (map_numbered_ss_seq),
    this function returns original and obtained products"""
    reactants, products_original = [Chem.MolFromSmiles(smi) for smi in rxn.split(">>")]
    ...
    try:
        products_obtained = neutralize_atoms(
            Chem.MolFromSmiles(Chem.MolToSmiles(Chem.rdmolops.RemoveAllHs(products_obtained, sanitize=False)))
        )
    except:  # noqa: E722
        return None
    return [Chem.MolFromSmiles(Chem.MolToSmiles(products_original)), products_obtained]


def compare(given: Mol, obtained: Mol) -> tuple[list[Atom], list[str]]:
    ...
    for i, given_prod_atom_tag in enumerate(given_prod_atom_tags):
        for j, prod_tag in enumerate(prod_atom_tags):
            if given_prod_atom_tag != prod_tag:
                continue
...
def check_label(rxn: str, label: list[Any]) -> tuple[bool, str | None]:
    ...
    if result:
        ...
        diff = compare(
            neutralize_atoms(
                Chem.MolFromSmiles(Chem.MolToSmiles(Chem.rdmolops.RemoveAllHs(products_original, sanitize=False)))
            ),
            products_obtained,
        )[-1]
        if diff == []:  # the label achieved the transformation => done
            ...
            return True, Chem.MolToSmiles(products_obtained)
    else:
        return False, None  # invalid label due to arrow_pushing error


def get_ortho_para(mol: Mol, target_idx: int) -> list[int]:
    ...
    ring = []  # initialize to empty list to satisfy static analysis if loop doesn't break
    for ring_tuple in rings:
        if target_idx in ring_tuple:
...
def get_EWG_connected_atom(rmol: Mol, target_idx: int) -> tuple[int | list[int], list[int]]:
    ...
    atom_map_num = rmol.GetAtomWithIdx(
        target_idx
    ).GetAtomMapNum()  # map number of a ring atom that's connected to leaving group
    ...
    for pos in get_ortho_para(rmol, target_idx):  # index based ortho/para positions
        ...
        for neighbor in atom.GetNeighbors():
            if (
                rmol.GetBondBetweenAtoms(pos, neighbor.GetIdx()).GetBondType() == Chem.BondType.SINGLE
            ):  # Check if the bond is a single bond (not part of the aromatic ring)
                op_substituents.append((pos, neighbor.GetIdx()))
    for fg in EWGS:
        matches = rmol.GetSubstructMatches(Chem.MolFromSmarts(fg))  # index based matches for EWG in whole rmol
        for match in matches:
            for sub in op_substituents:
                if match[0] in sub:
                    EWG_connected_atom_idx = sub[0]
                    if (
                        EWG_connected_atom_idx in get_neighbor_props(rmol, atom_map_num, "index")
...
def get_acidic_EWG_map_nums(rmol: Mol, target_idx: int) -> list[int]:
