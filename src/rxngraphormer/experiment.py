...
if TYPE_CHECKING:
    ...
    class DataclassInstance:  # type: ignore[no-redef]
        pass

from .evaluation.classification_workflow import (
    ClassificationEvaluationSettings,
    evaluate_classification_splits,
    write_classification_csv,
    write_classification_json,
)
from .evaluation.regression_workflow import (
...
def jsonable(value: Any) -> Any:
    if is_dataclass(value):
        return jsonable(asdict(cast(DataclassInstance, value)))
...
def build_fit_manifest(
    *,
    ...
    exported_ckpt = getattr(artifacts, "exported_checkpoint_path", None)
    return {
        "schema_version": 1,
        ...
        "optimization": jsonable(getattr(artifacts.lit_module, "optimization_plan", None)),
        "checkpoint": {
            "best_model_path": exported_ckpt or best_ckpt,
            "trainer_best_model_path": best_ckpt,
...
def run_post_fit_classification_eval(
    *,
    artifacts: Any,
    config_path: str | os.PathLike | None,
    checkpoint_path: str | os.PathLike,
    ...
    report_dir = Path(output_dir) / "reports"
    results = evaluate_classification_splits(
        ClassificationEvaluationSettings(
            model_path=artifacts.root_dir,
            config_path=config_path,
            checkpoint_path=checkpoint_path,
    ...
    json_path = report_dir / "eval.json"
    csv_path = report_dir / "eval.csv"
    write_classification_json(results, json_path)
    write_classification_csv(results, csv_path)
    return {
        "json": str(json_path),
        "csv": str(csv_path),
        "checkpoint_path": str(checkpoint_path),
