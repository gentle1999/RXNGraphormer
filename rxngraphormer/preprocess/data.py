...
def _raw_file_sort_key(path):
    stem = os.path.splitext(os.path.basename(path))[0]
    suffix = stem.rsplit("_", 1)[-1]
    return (0, int(suffix), stem) if suffix.isdigit() else (1, stem)

def _matched_raw_files(root, name_regrex):
...
def _collated_data_count(slices):
    if slices is None:
        return 1
    first_slices = next(iter(slices.values()))
    return len(first_slices) - 1

def _separate_collated_data(data, slices, idx):
    if slices is None:
        if idx != 0:
            raise IndexError(f"Single-item processed block only supports idx=0, got {idx}")
        return data.clone() if hasattr(data, "clone") else data
    return separate(
        cls=data.__class__,
        batch=data,
        idx=idx,
...
def _ext_feat_tensor(ext_feat_desc):
...
def calc_batch_graph_distance(batch, edge_index, task, max_nodes_per_graph=512):
    ...
    for i in range(num_graphs):
        ...
        unreachable = 1_000_000
        dist = torch.full((graph_len, graph_len), unreachable, dtype=torch.int32, device=batch.device)
        dist.fill_diagonal_(0)

        edge_mask = (edge_index[0] >= start) & (edge_index[0] < end) & (edge_index[1] >= start) & (edge_index[1] < end)
...
class RXNG2SDataset(InMemoryDataset):
    def __init__(self, root, src_file, tgt_file, vocab_file, max_length=1024,
        ...
        self.data, self.slices = load_legacy_torch(self.processed_paths[0])
    ...
    def process(self):
        ...
        if not self.data_list:
            raise ValueError(f"No valid sequence graph data was generated from {self.src_file} and {self.tgt_file}")
...
class MultiRXNDataset(InMemoryDataset):
    def __init__(self, root, name_regrex='pretrain_rxn_dataset_test_0_*.csv', raw_data=[],transform=None, pre_transform=None,
        ...
        for processed_path in self.processed_paths:
            data, slices = load_legacy_torch(processed_path)
            self.data_lst.append(data)
            self.slices_lst.append(slices)
            self.data_num_lst.append(self.data_num_lst[-1]+_collated_data_count(slices))
    ...
    def process(self):
        for idx,raw_data_f in enumerate(self.raw_data_files):
            ...
            for rxn_smi_tgt in tqdm(rxn_smi_tgt_lst):
                ...
                data = Data(x=x_merge,edge_index=edge_index_merge,edge_attr=edge_attr_merge,
                            mol_index=mol_index,y=tgt_,ext_feat=_ext_feat_tensor(ext_feat_desc))
                data_list.append(data)
            if not data_list:
                raise ValueError(f"No valid reaction graph data was generated from {raw_data_f}")
    ...
    def get(self,idx):
        ...
        return _separate_collated_data(self.data_lst[blk_i], self.slices_lst[blk_i], idx)
    ...
    def process(self):
        ...
        if self.multi_process:
            ...
            for rxn_inf in results:
                ...
                data = Data(x=x_merge,edge_index=edge_index_merge,edge_attr=edge_attr_merge,
                                mol_index=mol_index,atom_mass=atom_mass_merge,y=tgt_,ext_feat=_ext_feat_tensor(ext_feat_desc))
                self.data_list.append(data)
        else:
            for rxn_smi_tgt in tqdm(rxn_smi_tgt_lst):
                rxn_inf = get_rxn_pfm_info((rxn_smi_tgt, self.task, self.ext_feat, self.ext_feat_type, self.ext_feat_param, self.mul_ext_readout))
        ...
        if not self.data_list:
            raise ValueError(f"No valid reaction graph data was generated from {self.name}")
        data, slices = self.collate(self.data_list)
        print(f'[INFO] {len(self.data_list)} Saving...')
        torch.save((data, slices), self.processed_paths[0])
...
def get_rxn_pfm_info(rxn_smi_tgt_task_ens):
    ...
    if rxn_inf is None:
        return None
    x_merge,edge_index_merge,edge_attr_merge,atom_mass_merge,x_oh_merge,edge_oh_attr_merge,a_graphs_merge,b_graphs_merge,mol_index,ext_feat = rxn_inf
